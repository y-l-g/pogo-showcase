...
func (b *Bootstrap) prepareEnvironment() (string, error) {
	...
	defaults := map[string]string{
		...
		"REVERB_APP_ID":        values["REVERB_APP_ID"],
		"REVERB_APP_KEY":       values["REVERB_APP_KEY"],
		"REVERB_APP_SECRET":    values["REVERB_APP_SECRET"],
...
func loadRuntimeEnv(values map[string]string) error {
	for key, value := range values {
		...
		if err := os.Setenv(key, value); err != nil {
			return fmt.Errorf("set %s from runtime env: %w", key, err)
		}
	}
	return nil
}
...
func writeEnvFile(path string, values map[string]string) error {
	...
	out.WriteString("# Process environment variables override values in this file.\n")
	for _, key := range keys {
		out.WriteString(key)
		out.WriteString("=")
